	...
	<head>
		...
		$(function(){
			...
			$("button[name='search']").click(function(e){//搜索按钮
				$("#search").attr("action","<%=request.getContextPath()%>/contract/contractNew_probationList.html");
		...
		function toRegular(){
			var content = '<form id="form" method="post" target="frame" action="<%=request.getContextPath()%>/contract/contractNew_toRegular.html">';
		...
		function regular(){
			showWindow("合同转正","<%=request.getContextPath()%>/contract/contractNew_regular.html","495","280");
		}
		...
		function getInputHtml(){
			var inputHTML = "";
			$("#list_body input,#list_body select").each(function(){
	...
	<body>
		...
		<div class="toolbox">
		    ...
		    <div class="buttonbox">
		        <ul>
		            <li>
		                <a onclick="return false;" class="btn_sh" href="#">
		                  试用期转正
		                </a>
		            </li>
		            <li>
		...
		<form id="search" method="post" action="<%=request.getContextPath()%>/contract/contractNew_probationList.html">
		    <!-- 查询条件 -->
