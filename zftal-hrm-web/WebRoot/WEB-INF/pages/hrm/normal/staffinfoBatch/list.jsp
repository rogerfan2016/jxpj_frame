...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<script type="text/javascript">
		    var heightOffset= ${heightOffset}>400?400:${heightOffset};
		    heightOffset+=230;
			$(function(){
				if(${closeField!=null&&closeField!=''}){
					var td=$("input[name='${closeField}']").closest("td");
		            var preTD=td.prevAll("td:first");
		            td.closest("td").append("<td></td><td></td>");
	                td.remove();
	                preTD.remove();
				}
				$("#list_body tr").dblclick(function(){
					...
					showWindow("修改","<%=request.getContextPath()%>/normal/staffBatch_modify.html?classId="+"${classId}&instanceId="+id+"&gh="+gh+"&editable="+editable, 750, heightOffset);
				});
				
				$("#btn_zj").click(function(){
					showWindow("增加","<%=request.getContextPath()%>/normal/staffBatch_add.html?classId="+'${classId}', 750,heightOffset);
				});
				...
				$("#btn_ckrz").click(function(){
                    ...
                    showWindowV1("变更日志","<%=request.getContextPath()%>/normal/staffBatch_showLog.html?classId=${classId}&instanceId="+id, 700, 500);
                });
				...
				$("#btn_plxg").click(function(){
                    ...
                    showWindow("批量修改","<%=request.getContextPath()%>/normal/staffBatch_batchEditView.html?classId=${classId}&"+checked.serialize(), 500, 200);
                });
				$("#btn_sc").click(function(){
					...
					$("#why_sure").click(function(){
						$.post("<%=request.getContextPath() %>/normal/staffBatch_remove.html?classId=${classId}&instanceId="+id+"&gh="+gh, null, function(data){
			  ...
			  $(".btn_dc").click(function(e){
		            ...
		            window.open("<%=request.getContextPath()%>/normal/staffBatch_export.html?classId=${classId}&"+str);
		      });
		      
		      $(".btn_dr").click(function(e){
		      		showWindow("数据导入", "<%=request.getContextPath()%>/baseinfo/infoClassData_upload.html?classId=${classId}", 480, 150 );		            
		      }); 			      
			...
			for ( var i=0; i<xmtrs.length; i++){
			    ...
			    //var html1='<%=request.getContextPath()%>'+"/normal/staffBatch_modify.html?instanceId="+id+"&gh="+gh+"&classId=${classId}";
			...
			//监听单击行
             $("tbody > tr[name='list_tr']").click(function(){           
            	...
            	selectAllOrCancel=function(obj,name){//全选选择框操作
		...
		function reflashPage(){
			...
			//$("#selectForm").attr("action","<%=request.getContextPath()%>/normal/staffBatch_list.html?classId="+"${classId}&modeType=right");
			$("#selectForm").submit();
		}
			...
			<div class="toolbox">
				<div class="buttonbox">
					<ul>
                            ...
                            <li>
                                <a id="btn_ckrz" class="btn_ck">查看日志</a>
                            </li>
					</ul>
				</div>
			</div>
			<form id="selectForm" enctype="application/x-www-form-urlencoded" method="post" >
				...
				<div class="searchtab">
					<table width="100%" border="0">
						...
						<%--
							<tr>
								<td>职工号</td>
								<td>
									<input type="text" name="overall.userId" class="text_nor" style="width: 90px;" value="${overall.userId }"/>
									<!-- <input type="text" name="modeType" class="text_nor" style="display: none;" value="${modeType }"/> -->
								</td>
								<td>
									姓  名
								</td>
								<td>
									<input type="text" name="overall.userName" class="text_nor" style="width: 80px;" value="${overall.userName }"/>
								</td>
								<td>当前状态</td>
								<td>
									<ct:codePicker name="overall.status" catalog="<%=ICodeConstants.STATUS %>" code="${overall.status }" width="90" />
								</td>
							...
							<tr>
								<td>部  门</td>
								<td>
									<ct:codePicker name="overall.departmentId" catalog="<%=ICodeConstants.DM_DEF_ORG %>" code="${overall.departmentId }" width="230" />
								</td>
								<td>编制类别</td>
								<td>
									<ct:codePicker name="overall.formationType" catalog="<%=ICodeConstants.AUTH_TYPE %>" code="${overall.formationType }" width="180" />
								</td>
								<td></td>
								<td></td>
							</tr> --%>
							...
							<tr>
								<th>性 　别</th>
								<td>
									<ct:codePicker name="overall.sex" catalog="<%=ICodeConstants.SEX %>" code="${overall.sex }" width="60"/>
								</td>
								<th>岗位类别</th>
								<td>
									<ct:codePicker name="overall.postId" catalog="<%=ICodeConstants.DM_DEF_WORKPOST %>" code="${overall.postId }" width="100" />
								</td>
								<th>编制类别</th>
								<td>
									<ct:codePicker name="overall.formationType" catalog="<%=ICodeConstants.AUTH_TYPE %>" code="${overall.formationType }" width="100" />
								</td>
								<th>行政职务级别</th>
								<td>
									<ct:codePicker name="overall.dutyLevel" catalog="<%=ICodeConstants.ADMIN_DUTY_LEVEL %>" code="${overall.dutyLevel }" width="100" />
								</td>
							</tr>
							<tr>
								<th>专业技术职务级别</th>
								<td>
									<ct:codePicker name="overall.postTitle" catalog="<%=ICodeConstants.POST_TITLE %>" code="${overall.postTitle }" width="100" />
								</td>
								<th>进校时间</th>
								<td colspan="5">
					          		<input type="text"  name="overall.startDate" id="startDate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate" value="${overall.startDateString}" style="width: 100px;"/>  
					          			至
					            	<input type="text"  name="overall.endDate" id="endDate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate" value="${overall.endDateString}" style="width: 100px;"/>
					         	</td>	
							</tr>
							 --%>
						</tbody>
					...
					<div class="comp_con">	
						...
						<!-- 伸缩按钮 -->
						<div class="more--item_bottom" style="clear:both;margin-bottom:5px"><p><a href="#" class="down">查询条件</a></p></div>	
					</div>
					<table width="100%" border="0">
						<tfoot>
				          <tr>
					...
					<thead id="list_head">
						<tr>
							...
							<c:forEach items="${viewables}" var="p">
							    <c:if test="${p.fieldName != 'gh' and p.fieldName != 'xm' and p.fieldName != 'dwm' and p.fieldName != 'dqztm' and p.fieldName != 'bzlbm'}">
						...
						<c:forEach items="${staffInfoDtos}" var="staffInfoDto">
							<tr name="list_tr" id="${staffInfoDto.dynaBean.values['globalid']}" gh="${staffInfoDto.overall.values['gh']}">
								...
								<c:forEach items="${viewables}" var="infoProperty">
								    <c:if test="${infoProperty.fieldName != 'gh' and infoProperty.fieldName != 'xm' and infoProperty.fieldName != 'dwm' and infoProperty.fieldName != 'dqztm' and infoProperty.fieldName != 'bzlbm'}">
