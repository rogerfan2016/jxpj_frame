	...
	<script type="text/javascript">
		$(function(){
			...
			$("input[name='checkItem']").click( function(){
				...
				$.post("<%=request.getContextPath() %>/baseinfo/formInfoMember_open.html?batch=1", "name=${name}&classId=" + classid + "&open=" + open, function(data){});
			});
		...
		function property( classid ) {
			$("form:first").attr("action", "<%=request.getContextPath() %>/baseinfo/formInfoMemberProperty_list.html?batch=1");
		...
		function modify( classid ) {
			...
			showWindow("修改","<%=request.getContextPath() %>/baseinfo/formInfoMember_edit.html?batch=1&name=${name}&classId="+classid, 480, 260);
		}
		...
		function swap( prev, next ) {
			...
			$.post('<%=request.getContextPath() %>/baseinfo/formInfoMember_swap.html?batch=1', params, function(data){
				if( data.success ) {
					prev.before(next);
				}
			}, "json")
...
<body>
	<form action="<%=request.getContextPath() %>/baseinfo/formInfoMember_list.html" method="post">
		...
		<input type="hidden" name="member.batch" value="1" />
	</form>
