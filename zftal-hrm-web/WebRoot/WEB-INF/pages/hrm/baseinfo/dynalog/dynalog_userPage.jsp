	...
	<script type="text/javascript">
		$(function(){
			...
			$("button[name='search']").click(function(e){//搜索按钮
				$("#log").attr("action","<%=request.getContextPath()%>/baseinfo/dynalog_userPage.html");
				$("#log").attr("method","post");
				$("#log").submit();
				e.preventDefault();//阻止默认的按钮事件，防止多次请求
			});
			$("#back").click(function(){
				window.location.href = _path + "/normal/staffInfo_list.html?classId=${classId}";
			});
			operationList();
		...
		function operationList(){
			...
			$("a[name='detail']").click(function(){
				...
				window.location.href=_path+"/baseinfo/dynalog_userDetail.html?logId="+logId+"&classId="+classId;
			});
...
<form action="baseinfo/dynalog_userPage.html" method="post" id="log">
	<div class="searchtab">
      ...
      <tbody>
        <tr>
          ...
          <td>
          	...
          	<input type="hidden" name="gh" class="text_nor" style="width:150px" value="${gh }"/>
          	<input type="hidden" name="classId" class="text_nor" style="width:150px" value="${classId }"/>
          </td>
        </tr>
      </tbody>
      <tfoot>
