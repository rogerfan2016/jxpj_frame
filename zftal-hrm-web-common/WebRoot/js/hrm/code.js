...
function codePicker(obj,catalogId){
    ...
    function loadChild(catalogId,parentId,obj, level){
    	...
    	var subId = "#" + $(obj).closest("div").find("[type='hidden']").attr("name") + "_sub";
    	var param = "catalogId="+catalogId+"&parentId="+parentId;
    	if ($.trim($(subId).val()) != null) {
    		param += "&suborgscope=" + $.trim($(subId).val());
    	}
    	$.post(_path+'/code/codeItem_getChildren.html',param,function(data){
			dataProcess(data);
			if(parentId == ''){
    	...
    	function createNodePanel(data){
    		...
    		function createNodeItem(node){
    			var html="";
    			if(node.dumped==0){
					//$("#input_sel_con").append("<li><a style=\"cursor:pointer;\">"+node.description+"</a></li>");
    				html = "<li><a style=\"cursor:pointer;\">"+node.description+"</a></li>";
    			}
				if(node.dumped==1){//如果条目废弃，显示红色
					//$("#input_sel_con").append("<li><a style=\"cursor:pointer;\" title=\"已废弃\"><font color=\"red\">"+node.description+"</font></a></li>");
					html = "<li><a style=\"cursor:pointer;\" title=\"已废弃\"><font color=\"red\">"+node.description+"</font></a></li>";
				}
				//var v = $("#input_sel_con").find(":last-child>a");
    ...
    function initInput(obj,valueObj,catalogId){//初始化表单控件
    	...
    	function confirmButtonEvent(obj){
			...
			$("#confirm").click(function(e){
				...
				$("#_codeSelector>#inputsel").hide();
			});
    	}
    	
    	function bindCloseButtonEvent(){
    		$("#close").click(function(){
    			if($("#_codeSelector>#inputsel").is(":visible"))
    				$("#_codeSelector>#inputsel").hide();
    		});
    	}
    }
    ...
    function showPanel(obj){
    	...
    	if($("#_codeSelector>#inputsel").is(":hidden")){
    		$("#_codeSelector>#inputsel").slideDown("fast");
    	}
    }
    
}
function initSearchLayer(){
	$("#_codeSelector>#inputsel").click(function(e){
		e.stopPropagation();
	});
