		...
		<script language="javascript">
		    ...
		    function gotoPage(pageNumber) {
		    	if (!isNumber($("#pageSize").val())) {
		        	$("#pageSize").val(${model.perPageSize });
		        }
		    ...
		    function changePageSize(pageNumber,pageSize){
		        ...
		        if (!isNumber($("#biblepage").val())) {
		        	$("#biblepage").val(pageNumber);
		        }
		        gotoPage($("#biblepage").val());
			}
							...
							<td>
								<table border="0" cellspacing="4"  align="right" style="border-collapse: separate;">
									<tr>
									  	<td>
									  		每页显示
									  		<input id="pageSize" name="perPageSize" value="${model.perPageSize }" onchange="changePageSize(${model.toPage },${model.perPageSize });" size="2"/>条记录
									    </td>
									    <td>/共
									    ...
									    <td nowrap="nowrap">跳转到 <input id="biblepage" value="<%=pg.getPage() %>" type="text" onchange="changePageSize(${model.toPage },${model.perPageSize });" size="2" />&nbsp;页</td>
									    <td><a href="javascript:void(0);"   class="text" onClick="turnPage(this);return false;" id="gopage"><span>跳转</span></a></td>
								    </tr>
								    <input type="hidden" id="toPage" name="toPage" value="${model.toPage }"/>
								</table>
							</td>
						</tr>
					</table>
